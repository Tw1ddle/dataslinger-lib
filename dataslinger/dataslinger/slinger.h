...
public:
    ...
    HeterogeneousMap(const std::map<T, nonstd::any>& data) : m_map{data} {}
    ~HeterogeneousMap() = default;
    ...
    nonstd::any& operator[](const T&& key)
    {
        return m_map[key];
    }
    ...
    template<typename V> bool hasTypedValue(const T key) const
    {
        ...
        try {
            nonstd::any_cast<V>(m_map.at(key));
    ...
    {
        return nonstd::any_cast<V>(m_map.at(key));
    }
